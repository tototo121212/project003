						...
						<c:choose>
							...
							<c:otherwise>
								...
								<tr>	
									...
									<td>${k.c_brand}</td>
									<td>${k.c_employee}</td>
									<td><a style="list-style: none;" href="one_customer_list?c_idx=${k.c_idx}&cPage=${cPage}">${k.c_subject}</a></td>
									<td>
									<c:choose>
						...
						<td colspan="6">
							<ol class="paging">
								<c:choose>
									...
									<c:otherwise>
										<li><a href="costomer_list?cPage=${pvo.beginPage -pvo.pagePerBlock}">이전으로</a></li>
									</c:otherwise>
								</c:choose>
								<c:forEach var="k" begin="${pvo.beginPage}" end="${pvo.endPage}" step="1">
									<c:choose>
										...
										<c:otherwise>
											<li><a href="costomer_list?cPage=${k}">${k}</a></li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								<c:choose>
									...
									<c:otherwise>
										<li><a href="costomer_list?cPage=${pvo.beginPage + pvo.pagePerBlock}">다음으로</a></li>
									</c:otherwise>
								</c:choose>	
							</ol>
						</td>
